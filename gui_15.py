...
class HashTable:
    ...
    def insert(self, key, value):
        ...
        for kv in self.table[index]:
            if kv[0] == key:
                kv[1] = value
                return f"Updated {value} at index {index} for key {key}"
        self.table[index].append([key, value])
        return f"Inserted {value} at index {index} for key {key}"
    ...
    def delete(self, key):
        ...
        for i, kv in enumerate(self.table[index]):
            if kv[0] == key:
                ...
                return f"Deleted {deleted_value} from index {index} for key {key}"
        return "No element found for that key."

    def search(self, key):
        index = self.hash_function(key)
        for kv in self.table[index]:
            if kv[0] == key:
                return f"Found {kv[1]} at index {index} for key {key}"
        return "No element found for that key."

    def traverse(self):
        return [
            f"{index}: {', '.join([str(kv) for kv in values])}" if values else f"{index}: " 
...
class HashTableApp(ctk.CTk):
    def __init__(self, hash_table_size):
        ...
        self.title("Hash Table GUI with Chaining")
    ...
    def display_size(self):
        size = sum(len(bucket) for bucket in self.hash_table.table)
